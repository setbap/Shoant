	...
	changeLike(e) {
		...
		axios
			...
			.then((res) => {
				...
				message.success(
					`you ${this.state.like ? "like" : "unlile"} this`,
				);
			})
			.catch((err) => {
				if (!err.response.status) {
					message.error("can not connect to internet");
				} else if (err.response.status === 422) {
					message.error("wrong info");
				} else if (err.response.status === 404) {
					message.error("product not foound");
				} else {
					message.error("unknown err");
				}
			});
	}
